...
module.exports =
  justExport: (SceneManager) ->
    ...
    for child in children
      ...
      if child.isVoxel
        ...
        if child.material?.color
          ...
          throw new Error('ERROR: Cannot use textured cubes when creating a voxel')

        else if child instanceof THREE.Object3D # Like a ladder
          throw new Error('ERROR: Cannot use other voxels when creating a voxel')
    ...
    ex = new THREE.GeometryExporter()
    json = ex.parse(colorGeo)
    json



  exportAndReplace: (SceneManager) ->
    colorGeo = new THREE.Geometry()
